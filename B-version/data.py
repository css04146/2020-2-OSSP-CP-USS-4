...
def dec_target_processing(value, dictionary):
    ...
    # 넘파이 배열에 인덱스화된 배열과 그 길이를 전달
    return np.asarray(sequences_target_index), np.asarray(sequence_length) # 수정
...
def train_input_fn(train_input_enc, train_target_dec_length, train_target_dec, # 수정
batch_size):
...
def load_vocabulary():
    ...
    char2idx, idx2char = make_vocabulary(vocabulary_list) # 수정
    # 두 가지 형태의 키와 값이 있는 형태를 리턴
    # (예) 단어: 인덱스, 인덱스: 단어
    return char2idx, idx2char, len(char2idx) # 수정
...
def main(self):
    char2idx, idx2char, vocabulary_length = load_vocabulary() # 수정
